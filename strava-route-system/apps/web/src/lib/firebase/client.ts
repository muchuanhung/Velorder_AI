...
function getAuthDomain() {
  if (typeof window !== "undefined") {
    return window.location.host; // host 含 port（非 80/443 時）
  }
  return process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN ?? process.env.NEXT_PUBLIC_VERCEL_URL ?? "";
}

const firebaseConfig = {
  apiKey: process.env.NEXT_PUBLIC_FIREBASE_API_KEY,
  authDomain: getAuthDomain() || process.env.NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN,
